...
export interface PaperWatchItem { id: number; stock: Stock; display_order: number; added_at: string; latest_snapshot?: Snapshot | null; latest_advice?: Advice | null; }
...
export interface PaperAccount { id: number; owner_name: string; phone?: string | null; masked_phone?: string | null; initial_cash: number; cash_balance: number; cash_available: number; cash_frozen: number; status: string; created_at?: string | null; last_login_at?: string | null; reset_at?: string | null; }
export interface PaperAccountCaptcha { captcha_id: string; phone: string; captcha_code: string; expires_in: number; }
...
export interface PaperEquityPoint { id: number; snapshot_time: string; cash_balance: number; cash_frozen: number; position_market_value: number; total_assets: number; net_value: number; daily_return_pct?: number | null; benchmark_code?: string | null; benchmark_value?: number | null; }
export interface PaperQuote { stock_id: number; code: string; name: string; market: string; price: number; price_source: string; is_realtime: boolean; limit_up?: number | null; limit_down?: number | null; previous_close?: number | null; limit_rate_pct?: number | null; strategy_signal?: Signal | null; strategy_confidence?: number | null; }
export interface PaperPosition { stock_id: number; code: string; name: string; market: string; total_quantity: number; available_quantity: number; today_buy_quantity: number; frozen_quantity: number; avg_cost: number; market_price: number; price_source: string; market_value: number; floating_pnl: number; floating_pnl_pct: number; asset_ratio_pct: number; strategy_signal?: Signal | null; strategy_confidence?: number | null; }
...
export interface PaperAdminSession { token: string; admin: { username: string }; }
export interface PaperAdminAccountSummary { account_id: number; owner_name: string; status: string; initial_cash: number; cash_balance: number; cash_available: number; cash_frozen: number; position_market_value: number; total_assets: number; position_count: number; open_order_count: number; trade_count: number; flow_in: number; flow_out: number; net_flow: number; flow_count: number; last_flow_at?: string | null; created_at?: string | null; last_login_at?: string | null; }
export interface PaperAdminCashFlow extends PaperCashFlow { account_id: number; owner_name: string; code?: string | null; name?: string | null; }
export interface PaperAdminTotals { account_count: number; initial_cash: number; cash_balance: number; cash_available: number; cash_frozen: number; position_market_value: number; total_assets: number; flow_in: number; flow_out: number; net_flow: number; flow_count: number; trade_count: number; open_order_count: number; }
export interface PaperAdminOverview { admin: { username: string }; accounts: PaperAdminAccountSummary[]; totals: PaperAdminTotals; flows: Paged<PaperAdminCashFlow>; flow_types: string[]; }
