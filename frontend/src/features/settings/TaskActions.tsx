...
export default function TaskActions({ loading, runTask }: { loading: boolean; runTask: (task: 'bootstrap' | 'market' | 'history' | 'missingDailyKline' | 'intraday' | 'news' | 'simplifyNews') => void }) {
  return (
    <Card title="真实数据同步" className="section-gap">
      <Space wrap>
        ...
        <Button icon={<PlayCircleOutlined />} loading={loading} onClick={() => runTask('missingDailyKline')}>补全缺失日 K</Button>
